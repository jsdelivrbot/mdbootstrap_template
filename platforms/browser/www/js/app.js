window.dao =  {
    ...
    syncURL: "http://localhost/offline-sync/api/employees",

    initialize: function(callback) {
        var self = this;
        this.db = window.openDatabase("syncdb", "1.0", "Sync Demo DB", 200000);
    ...
    applyChanges: function(employees, callback) {
        this.db.transaction(
            function(tx) {
                ...
                var sql =
                    "INSERT OR REPLACE INTO employee (id, firstName, lastName, email, officePhone, deleted, lastModified) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?)";
                log('Inserting or Updating in local database:');
                var e;
                for (var i = 0; i < l; i++) {
                    e = employees[i];
                    log(e.id + ' ' + e.firstName + ' ' + e.lastName + ' ' + e.email + ' ' + e.officePhone + ' ' + e.deleted + ' ' + e.lastModified);
...
function sideNav(){
    ...
    $('#slide-out ul li > a').click(function(e){
        e.preventDefault();
        $("#main").load($(this).attr('href'));
        $("#slide-out").css("transform", "translateX(-250px)");
